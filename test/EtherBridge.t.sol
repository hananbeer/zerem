...
contract BridgeTest is Test {
    ...
    function testBridgeLock() public {
        ...
        Vm.Log memory entry = entries[0];
        assertEq(entries.length, 2);
        assertEq(entry.topics[0], keccak256("TransferLocked(address,uint256,uint256)"));
        assertEq(uint256(entry.topics[1]), uint256(uint160(address(this))));

        (uint256 amountLocked, uint256 lockTimestamp) = abi.decode(entry.data, (uint256, uint256));
        assertEq(amountLocked, amount);
        assertEq(lockTimestamp, uint256(block.timestamp));

        Zerem zerem = bridge.zerem();
    ...
    function testBridgeLockAndUnlock() public {
        uint256 amount = 1000e18;
        ...
        assertEq(entries.length, 2);
        // console.logBytes32(entry.topics[0]);
        // console.logAddress(address(this));
        // console.logBytes32(entry.topics[1]);
        assertEq(entry.topics[0], keccak256("TransferLocked(address,uint256,uint256)"));
        assertEq(uint256(entry.topics[1]), uint256(uint160(address(this))));
